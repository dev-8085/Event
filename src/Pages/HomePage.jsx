...
// Mock events data with enhanced properties
const events = [
  {
    id: 1,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
            {events.map((event, index) => (
              <div key={event.id} className="group cursor-pointer" onClick={() => window.location.href = `/event/${event.id}`}>
                <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-lg border border-white/20 transition-all duration-500 hover:scale-105 hover:rotate-1 hover:shadow-2xl hover:shadow-purple-500/25 ${event.featured ? 'ring-2 ring-purple-400/30' : ''}`}>
                  ...
                  <div className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-purple-500 via-pink-500 to-blue-500 p-[2px]">
                      <div className="w-full h-full rounded-3xl bg-gradient-to-br from-slate-900/90 to-purple-900/90"></div>
                    </div>
                  </div>
                </div>
              </div>
